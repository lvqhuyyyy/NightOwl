const ExeGrammar = require('../models/ExeGrammar');

const getExeGrammar = async (req, res) => {
    try {
        const { section } = req.query;
        
        if (section) {
            // If a section is specified, fetch TestGrammar by section
            const testGrammar = await ExeGrammar.find({ sections: section });
            res.status(200).json({ data: testGrammar, message: 'success' });
        } else {
            // If no section is specified, return distinct sections
            const testGrammarSections = await ExeGrammar.distinct('sections');
...
const submitAnswer = async (req, res) => {
    ...
    await Promise.all(
        Object.entries(values).map(async ([questionId, userAnswer]) => {
            const question = await ExeGrammar.findOne({ _id: questionId });
...
module.exports = {getExeGrammar, submitAnswer}